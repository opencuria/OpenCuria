...
(CVE-2026-44545). Computer-use PNG screenshots and video payloads are
sent as base64 over this connection and exceed that, which drops the
runner WebSocket. File transfers are chunked (see Section 7). Keep
Daphne (`DAPHNE_WEBSOCKET_MAX_MESSAGE_SIZE` /
...
(`process_*`) exactly like `build`. In plan mode, ambiguities must be
resolved with the `question` tool before the plan is written — never as
open questions in the plan text. Global ask gates that survive
